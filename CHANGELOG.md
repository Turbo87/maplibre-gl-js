...
### 🐞 Bug fixes
- Fix banding and stair-stepped shading in `hillshade` layers on mobile GPUs by reading the DEM texture coordinate at `highp` instead of the fragment shader's default `mediump`, which was quantising it to a fraction of a DEM sample when the source is overzoomed (by [@Turbo87](https://github.com/Turbo87))
