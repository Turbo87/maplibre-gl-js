...
function drawLayerSymbols(
    ...
    // A data-driven `icon-rotation-alignment` is only honoured where the alignment is expressed as
    // this shader rotation in the first place; `SymbolStyleLayer` has already ruled out the layer
    // setups where it is not, so those keep using the layer-wide value.
