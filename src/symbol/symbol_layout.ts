...
function addFeature(bucket: SymbolBucket,
    ...
    // `icon-rotation-alignment` can vary per feature, so it is resolved here and recorded per
    // vertex; the renderer reads it back from `a_rotate_symbol`.
    const iconRotateWithMap = bucket.layers[0].getIconRotateWithMap(feature, canonical);
