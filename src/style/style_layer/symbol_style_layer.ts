...
export class SymbolStyleLayer extends StyleLayer {
    ...
    /**
     * Whether `icon-rotation-alignment` is a data expression *and* this layer is configured so that
     * the per-feature value can actually be honoured. When true, icon buckets carry a per-vertex
     * `a_rotate_symbol` attribute and the shader uses it instead of the `u_rotate_symbol` uniform.
     */
    ...
    /**
     * The feature's `icon-rotation-alignment`, as the boolean the renderer works in: whether the
     * icon rotates with the map. A constant `auto` is resolved by {@link recalculate}; a data
     * expression can still evaluate to `auto` per feature, so resolve that here too.
     */
