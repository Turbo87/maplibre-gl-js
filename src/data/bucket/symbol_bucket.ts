...
export class SymbolBuffers {
    ...
    /**
     * Per-vertex `icon-rotation-alignment`, only present on icon buffers of layers where that
     * property is a data expression. Left undefined otherwise, in which case the shader falls back
     * to the layer-wide `u_rotate_symbol` uniform.
     */
...
export class SymbolBucket implements Bucket {
    ...
    /**
     * @param rotateWithMap - the feature's resolved `icon-rotation-alignment`, recorded per vertex
     * when `arrays.rotationAlignmentVertexArray` exists. Irrelevant for text, which has no
     * data-driven rotation alignment.
     */
